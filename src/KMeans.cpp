...
bool comparator ( const pair<double,double>& l, const pair<double,double>& r)
...
double SortNGetLabel(vector<pair<double,double>> &distnlabel, vector<double> &labels, int k)
{
    int p=0,n=0;
...
double CalculateAccuracy(vector<double> &givenlabels, vector<double> &predictedlabels)
{
    ...
    cout<<correct<<"\t\t"<<givenlabels.size()<<endl;
    return (double) (correct/(double)(givenlabels.size()))* 100;
}

double CalculateKMeansAccuracy(table &traindata,table &testdata,vector<double> &labels,int k)
{
    vector<pair<double,double>> distnlabel;
    pair<double,double> distlbpair;
    ...
    for(int i=0;i<testdata.size();++i)
    {
        for(int j = 0; j < traindata.size(); ++j)
        {
            ...
            {
                dist += pow((traindata[j][l] - testdata[i][l]),2.0);
                dist = sqrt(dist);
            }
...
int main(int argc,char **argv)
{
    ...
    while(getline(file,line))
    {
        CreateAttributeTable(tempdata,line);
        for(int z = 0; z<tempdata.size();++z){
            for(int k = 0;k<tempdata[0].size()-1;++k){
                if(z==0){
                    sum_val.push_back(tempdata[z][k]);
                }
                else
                    sum_val[k]+=tempdata[z][k];
            }
        }
        for(int k = 0;k<tempdata[0].size()-1;++k){
            sum_val[k]=sum_val[k]/tempdata.size();
        }
        means.push_back(sum_val);
        sum_val.clear();
        tempdata.clear();
    }
    ...
    for(int i = 0; i<=9; ++i)
    {
        ...
        for(int k = 1; k<=10; ++k){
            ...
            cout<<"Accuracy "<<k<<" : "<<accuracy[k-1][i]<<endl;
        }
        labels.clear();
        subtraindata.clear();
        subtestdata.clear();
   ...
   // CreateAttributeTable(testdata,argv[argc-1]);
    Kvalue = 1;
    GetDistinctLabels(means,labels);
    
    finalaccuracy = CalculateKMeansAccuracy(means,testdata,labels,Kvalue);
